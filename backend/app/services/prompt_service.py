def create_analysis_prompt(summaries_data: str, conversations_data: str, messages_data: str, quizzes_data: str, bookmarked_summaries_data: str, bookmarked_quizzes_data: str) -> str:
    ...
    分析プロンプトを生成する。
    """
    prompt = f"""あなたは心理分析と学習支援の専門家です。以下の情報をもとに、このユーザーの全体像、強み、弱みを分析し、今後の成長のための具体的なアドバイスと、最適な学習コンテンツを推薦してください。Markdown形式で解答してください。
...
*   **過去の学習パターン:** 過去の学習内容、解答傾向、興味関心に基づいて、次に学習すべき内容を特定してください。
...
**分析する上で参考にしたデータ数:**（要約数、チャット数、メッセージ数、クイズ数、ブックマーク数など）
